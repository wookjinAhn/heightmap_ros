...
void pc_callback (const sensor_msgs::PointCloud2ConstPtr& pointcloud2_msg)
{
    ...
    std::vector<quadtree::Point3D*> inputQPoints = qt.ReadTopicToPoints(pointcloud_msg);
    std::vector<quadtree::Point3D*> samplingPoints = qt.SamplingPoints(inputQPoints, 5000);
